...
8.25
자문분들을 포함한 전체회의를 진행했다. 현실가능한 정도로 개발의 방향성이 조금 바뀌었다. 두분이 계셔서 참 다행이다. cto분과 개발도 조금 진행했다. 일단 필요한 개발환경을 모두 세팅했다. 로컬의 개발, 인스턴스의 배포까지 필요한 작업들을 해두고 ci도 만들었다. 스프레드시트에서 데이터를 가져오는 작업을 했다. 너무 기뻐서 날라갔다. 이 작업도 과정에 대한 기록을 잘 남겨두어야겠다.


8.26 수
다현님이 오신 날이다. 어제 맞춰놓은 개발셋팅을 온보딩하고 명령어도 알려주었다. 배운걸 바로 복습할 수 있어서 좋았다. 오후는 외근 미팅이 잡혀있어 최대한 오전 안으로 해결하도록 했다. 

8.27 목

코로나+태풍의 합작으로 재택근무를 했다. 협업에 필요한 기존 레가시의 개발/배포 절차를 문서화 시켜두었다. 또한 내일 어느 작업을 진행해야 인스턴스/로컬을 개발하기 좋은 환경으로 만들 수 있을지 고민하여 해결절차를 적어두었다. 다현님이 제안하신 이슈 템플릿, 머지 템플릿에 대해 피드백하고 각 질문에 대한 중요성을 인식하기 위해 좀 찾아봤다. 좋은 커밋 메세지를 작성하는데에 좀 시간을 들이면 좋을 것 같다. cto님이 격주 이터레이션 회의/스탠드업 회의/칸반보드 진행을 제안했다. 내일 얘기해보기로 했다.


8.28 금

- 레가시에 새로운 컴포넌트를 껴넣기 위해 GTM이라는걸 사용해봤다. 과정을 상기해 적어둘 필요가 있다.
- `git commit --amend`, `git rm -r --cache "filename"`(unstaging)을 처음 써봤다. 많이 보게될 명령어 아닐까?
- 커밋 메세지를 다채롭게 작성하기 위해 자료를 만들었다. 다른 개발자와 공유했다.
- 로컬에서 tomcat 자동 빌드/배포, 인스턴스에 war파일 카피, 인스턴스 내부 톰캣 하위로 이동시키기,기존경로 war 삭제까지 진행하는 스크립트를 만들었다. 편해 미처부러.
- SSO 진행을 위해 세션쿠키 만들고 히든키 공유하는 것 까지 진행했다. 이 또한 과정을 상기해 적어둘 필요가 있어보인다.
- cto님이 복잡한 상태관리 방식을 주제로 세미나를 진행하셨다. 내용을 정리해두었다.
