...
코로나+태풍의 합작으로 재택근무를 했다. 협업에 필요한 기존 레가시의 개발/배포 절차를 문서화 시켜두었다. 또한 내일 어느 작업을 진행해야 인스턴스/로컬을 개발하기 좋은 환경으로 만들 수 있을지 고민하여 해결절차를 적어두었다. 다현님이 제안하신 이슈 템플릿, 머지 템플릿에 대해 피드백하고 각 질문에 대한 중요성을 인식하기 위해 좀 찾아봤다. 좋은 커밋 메세지를 작성하는데에 좀 시간을 들이면 좋을 것 같다.
